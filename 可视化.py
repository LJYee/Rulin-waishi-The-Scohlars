...
df.rename(columns={
    ...
    '经度': 'lon',
    '纬度': 'lat',
    '城市出现次数': 'loc_total_freq',  # 地点总出现次数（原表字段）
    '涉及主要人物': 'characters',
    '活动类型': 'activity_type',
    '情节': 'plot_summary'
}, inplace=True)
...
st.title("《儒林外史》可视化分析")
st.titile(**"以The Scholar 第 10-29回为材料进行可视化" \
"分析地点与人物活动之间的关联，探究不同地点的人物活动特征")
# ---------------------- 标签页1：地点坐标地图 ----------------------
with tab1:
    st.title("《儒林外史》地点坐标地图")
    
    # 侧边栏筛选器（基于rename后的字段：chapter/location/activity_type）
    st.sidebar.header("地图筛选条件")
    ...
    if filtered_df.empty:
        ...
        for loc in filtered_df['location'].unique():
            ...
            # ---------------------- 关键统计（基于loc_total_freq字段） ----------------------
            # ① 总出现次数：按“地点+章回”去重后，求和loc_total_freq（避免多标签行重复累加）
            # 去重：同一地点同一章回只保留1行（防止多活动标签行重复统计次数）
            ...
            # ② 每章出现次数：读取去重后的loc_total_freq（原表预设的每回次数）
            ...
            # 标记大小：按总出现次数比例计算（12-32像素，避免过大/过小）
            # 先计算筛选后所有地点的最大总次数
            max_total_freq = max([
...
with tab2:
    ...
    if net_df.empty:
        ...
        # 节点大小映射：频次→大小（最小20，最大80，避免过小/过大）
        min_size = 20
        max_size = 80
        ...
        net = Network(
            directed=False,  # 无向图（人物-地点是双向关联）
            notebook=False,
            height="800px",  # 高度适配页面
            width="100%",    # 宽度占满页面
            bgcolor="#f8f9fa",  # 浅灰背景，提升视觉舒适度
            font_color="#333333"  # 字体颜色：深灰，避免刺眼
        )
        ...
        for _, row in net_df.iterrows():
            ...
            for char in row['filtered_chars']:
                ...
                # 边ID：按“人物-地点”排序，确保（A-B）和（B-A）是同一条边
...
# 
    ...
    # 1. 地点-活动图谱：数据筛选（复用侧边栏的“地点”“章回”筛选，新增“活动类型”筛选）
    # 新增活动类型筛选（仅用于地点-活动图谱，放在侧边栏人物筛选下方）
    ...
    if net_df_loc_act.empty:
        ...
        # ④ 布局与交互设置
        net_loc_act.barnes_hut(gravity=-2500, spring_length=200)  # 调整布局，避免节点重叠
        net_loc_act.show_buttons(["physics", "nodes", "edges"])  # 保留调节按钮
...
with tab3:
    ...
    # 2. 章回范围筛选（确保min_chapter/max_chapter已提前定义，来自df['chapter']的最值）
    min_chapter = int(df['chapter'].min())
    max_chapter = int(df['chapter'].max())
    ...
    if stat_dimension == "按地点统计":
        ...
        # 4. 合并“总出现次数”与其他统计信息（通过location关联）
    ...
    # ---------------------- 数据导出功能：适配修改后的“总出现次数”列 ----------------------
    st.subheader("数据导出")
    export_format = st.selectbox("选择导出格式", ["CSV", "Excel"])
    if st.button(f"导出当前统计表格（{export_format}）"):
        # 确保导出的表格与显示的表格一致
        export_table = display_table.copy()
        # 处理导出路径：替换为你电脑上的实际路径（如桌面），避免/mnt目录不存在
        export_path = f"/Users/ye/Desktop/儒林外史统计结果.{export_format.lower()}"
        ...
        try:
            ...
            with open(export_path, 'rb') as f:
                st.download_button(
                    label=f"点击下载{export_format}文件",
                    data=f,
                    file_name=f"儒林外史_{stat_dimension}_{selected_chapters_stat[0]}-{selected_chapters_stat[1]}回.{export_format.lower()}",
                    mime="text/csv" if export_format == "CSV" else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
        except Exception as e:
            st.error(f"导出失败：{str(e)}")
            st.info("建议检查路径是否存在，或尝试更换导出路径（如桌面）")
